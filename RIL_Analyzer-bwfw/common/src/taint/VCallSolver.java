...
public class VCallSolver {
    ...
    public void solve() {
        ...
        // hardcoded for some firmware without correct decompile result due to ghidra
        if(!finished){
     ...
     */
    public static Function getVFuncWithOffset(Namespace namespace, long offset) {
        String TAG = "[getVFuncWithOffset] ";
        Symbol symbol = null;
        if (VCallUtil.containVTable(namespace.getName())) {
            symbol = VCallUtil.getVTable(namespace.getName());
        }else{
            SymbolTable symtab = Global.getProgram().getSymbolTable();
        ...
        Data data = Global.getProgram().getListing().getDataAt(addr);
        if(data == null){
            RILLog.debugLog(TAG + "data at " + addr.toString() + " is null");
        } else {
            try {
                Address dataAddr = (Address) data.getValue();
                Function func = FunctionUtil.getFunctionWith(Global.getProgram(), dataAddr);
                return func;
